...
function generate_pom_release_bom {
	...
	do
		grep "/\(com\.liferay\.\|\)${artifact_file}/" /tmp/artifact_urls.txt | sort | while IFS= read -r artifact_url
		do
			# echo "${artifact_url}" >> "${_RELEASE_ROOT_DIR}/test-dependencies/artifact_url.txt"
